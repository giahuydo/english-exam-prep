...
const rawInterviewQuestions = [
  ...
  },
  {
    ...
    answer: { sections: [
      ...
      { id: 'reason', en: '*My main role is* **backend development**. / I work mainly with **NestJS** and **PostgreSQL**, / and recently I also worked a lot with **AI document processing**.', vi: '*Vai trò chính của mình là* **backend**. / Mình chủ yếu làm với **NestJS** và **PostgreSQL**, / và gần đây cũng làm nhiều về **xử lý tài liệu bằng AI**.' },
      { id: 'example', en: '*One important part was* the **OCR pipeline**. / When users upload a document, / the system needs to process it, run OCR, / and prepare the content for the next AI steps.', vi: '*Một phần quan trọng là* **OCR pipeline**. / Khi user upload tài liệu, / hệ thống cần xử lý, chạy OCR, / và chuẩn bị nội dung cho các bước AI tiếp theo.' },
      { id: 'problem', en: '*We had a problem because* / OCR was running on the **CPU worker**. / For large documents, / it was very slow / and sometimes caused **memory problems**.', vi: '*Tụi mình gặp vấn đề vì* / OCR đang chạy trên **CPU worker**. / Với tài liệu lớn, / nó rất chậm / và đôi khi gây **vấn đề memory**.' },
      { id: 'investigation', en: '*So first,* / I traced the whole processing flow / and checked the logs, memory usage, / and processing time.', vi: '*Vậy trước hết,* / mình trace cả luồng xử lý / và kiểm log, memory, / và thời gian xử lý.' },
      { id: 'cpu-fix', en: '*Then* I improved the CPU flow / with **smaller batches**, **checkpoint**, / and **recovery logic**.', vi: '*Sau đó* mình cải thiện luồng CPU / bằng **batch nhỏ hơn**, **checkpoint**, / và **recovery logic**.' },
      { id: 'gpu', en: '*After that,* / I worked with DevOps / to move the OCR inference / to a separate **GPU service**.', vi: '*Tiếp theo,* / mình phối hợp DevOps / để chuyển OCR inference / sang một **GPU service** riêng.' },
      { id: 'performance', en: '*This gave us a very good result.* / The processing time improved / from around **46 seconds per page** / to around **2 seconds per page**.', vi: '*Kết quả rất tốt.* / Thời gian xử lý cải thiện / từ khoảng **46 giây mỗi trang** / xuống còn khoảng **2 giây mỗi trang**.' },
      { id: 'strangeloop', en: '*Another part I am working on* / is an **agent workflow system** called **StrangeLoop**.', vi: '*Một phần khác mình đang làm* / là **hệ thống agent workflow** tên **StrangeLoop**.' },
      { id: 'tools', en: '*The main idea is* / instead of only asking the LLM to answer directly, / the workflow can use different **tools** / to search or read data from our system.', vi: '*Ý chính là* / thay vì chỉ bảo LLM trả lời trực tiếp, / workflow có thể dùng các **tool** khác nhau / để search hoặc đọc data từ hệ thống.' },
      { id: 'control', en: '*Then* the backend still controls the important parts, / such as **validation**, **permission**, / and **tool execution**.', vi: '*Sau đó* backend vẫn kiểm soát các phần quan trọng, / như **validation**, **permission**, / và **thực thi tool**.' },
      { id: 'devops', en: '*For this part,* / I also work with DevOps / to set up the infrastructure, / service connection, environment, / and authentication.', vi: '*Với phần này,* / mình cũng phối hợp DevOps / để setup hạ tầng, / kết nối service, environment, / và authentication.' },
      { id: 'overall', en: '*So overall,* / this project gives me experience / not only in normal backend development, / but also in **AI production systems**, / **GPU inference**, / **agent workflows**, / and **system reliability**.', vi: '*Tóm lại,* / dự án này cho mình kinh nghiệm / không chỉ backend thông thường, / mà còn **AI production system**, / **GPU inference**, / **agent workflow**, / và **độ ổn định hệ thống**.' },
    ] },
    contextIds: ['A', 'D', 'E'],
