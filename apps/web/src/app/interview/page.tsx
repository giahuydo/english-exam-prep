...
export default function InterviewPage() {
  ...
  const [selectedTopic, setSelectedTopic] = useState<MacroTopic | null>(null);
...
function ConnectionsView({ selected, setSelected, onQuestion }: { selected: MacroTopic | null; setSelected: (topic: MacroTopic | null) => void; onQuestion: (id: number) => void }) {
...
function MacroTopicPanel({ topic, onClose, onQuestion }: { topic: (typeof macroTopics)[number]; onClose: () => void; onQuestion: (id: number) => void }) { const questions = getQuestionsForMacroTopic(topic.id); return <section className="rounded-2xl border border-blue-200 bg-blue-50/50 p-4"><div className="flex items-start justify-between gap-3"><div><p className="text-[11px] font-bold uppercase tracking-widest text-blue-700">Macro topic · {topic.sourceContextIds.join(' + ')}</p><h3 className="mt-1 text-xl font-bold text-slate-950">{topic.title}</h3></div><button type="button" onClick={onClose} className="text-xs font-semibold text-slate-500">All topics</button></div><p className="mt-3 text-sm font-semibold text-slate-700">Path: {topic.summary}</p><p className="mt-2 text-xs text-slate-500">Triggers: {topic.keywords.join(' · ')}</p><div className="mt-4 flex flex-wrap gap-2">{questions.map((id) => <button key={id} type="button" onClick={() => onQuestion(id)} className="rounded-lg bg-white px-3 py-2 text-left text-xs font-semibold text-slate-700 shadow-sm">Q{id} <span className="font-normal text-slate-500">· {questionLabel(id)}</span></button>)}</div></section>; }
function TriggerDrill() { const [index, setIndex] = useState(0); const [show, setShow] = useState(false); const item = triggers[index]; return <section className="rounded-xl border border-slate-200 bg-white p-3"><p className="text-xs font-bold uppercase tracking-[0.16em] text-blue-700">10-second reaction drill</p><p className="mt-2 text-lg font-bold text-slate-950">“{item.phrases}”</p><button type="button" onClick={() => setShow(true)} className="mt-3 min-h-10 rounded-lg bg-slate-900 px-3 text-xs font-bold text-white">Reveal topic</button>{show && <p className="mt-3 text-sm text-slate-700">Think <b className="text-blue-700">{getMacroTopicsForTrigger(item.id).map((topic) => topic.title).join(' + ')}</b></p>}<button type="button" onClick={() => { setIndex((value) => (value + 1) % triggers.length); setShow(false); }} className="ml-3 min-h-10 px-2 text-xs font-semibold text-slate-500">Next</button></section>; }
