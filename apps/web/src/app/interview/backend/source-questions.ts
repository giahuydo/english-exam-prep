  ...
  1: [
    {
      ...
      answer: 'Basically, Node.js runs JavaScript on the main event loop. With I/O, like database or network requests, Node.js can wait without blocking the main thread. When I use async and await, only the current function pauses, while the event loop can continue handling other work. Some file and DNS operations can also use the libuv worker pool. CPU-heavy work is different. For example, heavy document processing or a large calculation can use a lot of CPU. If I run that work on the main thread, it can block the event loop, so other requests have to wait. For CPU-heavy work, I can use worker threads, a background queue, or a separate service, depending on the use case. So overall, Node.js works very well for I/O-heavy systems, but I avoid running heavy CPU work directly on the main thread.',
      keyIdea: 'I/O starts → await pauses current function → event loop stays free → I/O completes → function continues → heavy document processing / large calculation → CPU-heavy work blocks event loop → move heavy work away from the main thread',
    },
  ],
  2: [
    {
