...
function SpeakingAnswer({ answer }: { answer: string }) {
  ...
  return <div>
    ...
    <div aria-hidden="true" className="text-[15px] leading-9">
      {chunks.map((chunk, index) => <span key={index}>
        <span className={`box-decoration-clone rounded-md border-l-2 px-2 py-1 ${speakingAccents[index % speakingAccents.length]}`}>{chunk.trimEnd()}</span>
