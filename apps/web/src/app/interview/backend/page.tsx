...
import { speakingChunks, translatedSpeakingChunks } from './speaking-chunks';
...
function SpeakingAnswer({ answer, answerVi, activeRange, audio, audioKey, audioPath }: { answer: string; answerVi: string; activeRange: AudioRange | null; audio: ReturnType<typeof useInterviewAudio>; audioKey: string; audioPath: string }) {
  const chunks = speakingChunks(answer);
  const translations = translatedSpeakingChunks(answer, answerVi);
  const mapping = buildSpeechMapping(answer);
  const [openChunk, setOpenChunk] = useState<number | null>(null);
  const containerRef = useRef<HTMLDivElement>(null);
  useEffect(() => {
    if (openChunk === null) return;
    const dismissOutside = (event: PointerEvent) => {
      if (!containerRef.current?.contains(event.target as Node) || !(event.target as Element).closest('[data-speaking-chunk]')) setOpenChunk(null);
    };
    const dismissEscape = (event: KeyboardEvent) => {
      if (event.key === 'Escape') setOpenChunk(null);
    };
  ...
  let offset = 0;
  return <div ref={containerRef} className="text-[15px] leading-9">
    {chunks.map((chunk, index) => {
      const start = offset;
      offset += chunk.length;
      const text = chunk.trimEnd();
      const tooltipId = `${audioKey}-chunk-${index}-translation`;
      ...
      return <span key={index} data-speaking-chunk className="group relative">
        <button type="button" aria-expanded={openChunk === index} aria-controls={tooltipId} onClick={() => setOpenChunk(openChunk === index ? null : index)} className={`box-decoration-clone rounded-md border-l-2 px-2 py-1 text-left hover:cursor-help focus-visible:outline focus-visible:outline-2 focus-visible:outline-blue-500 ${speakingAccents[index % speakingAccents.length]}`}>
        ...
        <span id={tooltipId} role="tooltip" lang="vi" className={`fixed inset-x-3 bottom-[calc(1rem+env(safe-area-inset-bottom))] z-30 max-h-[50vh] overflow-y-auto rounded-lg border border-violet-300 bg-violet-100 px-3 py-2 text-sm leading-5 text-violet-950 shadow-lg [@media(hover:hover)]:absolute [@media(hover:hover)]:inset-x-auto [@media(hover:hover)]:bottom-full [@media(hover:hover)]:left-1/2 [@media(hover:hover)]:w-max [@media(hover:hover)]:min-w-full [@media(hover:hover)]:max-w-[min(28rem,90vw)] [@media(hover:hover)]:-translate-x-1/2 [@media(hover:hover)]:rounded-none [@media(hover:hover)]:border-0 [@media(hover:hover)]:bg-transparent [@media(hover:hover)]:p-0 [@media(hover:hover)]:pb-2 [@media(hover:hover)]:shadow-none ${openChunk === index ? 'visible pointer-events-auto opacity-100' : 'invisible pointer-events-none opacity-0'} [@media(hover:hover)]:group-hover:visible [@media(hover:hover)]:group-hover:pointer-events-auto [@media(hover:hover)]:group-hover:opacity-100 [@media(hover:hover)]:group-focus-within:visible [@media(hover:hover)]:group-focus-within:pointer-events-auto [@media(hover:hover)]:group-focus-within:opacity-100`}>
          <span className="flex items-center gap-2 [@media(hover:hover)]:rounded-lg [@media(hover:hover)]:border [@media(hover:hover)]:border-violet-300 [@media(hover:hover)]:bg-violet-100 [@media(hover:hover)]:px-3 [@media(hover:hover)]:py-2 [@media(hover:hover)]:shadow-lg">
            <span className="min-w-0 flex-1">{translations[index]}</span>
            <button type="button" lang="en" aria-label={`Play English chunk: ${text}`} onClick={() => audio.play({ text: mapping.text, src: `${audioPath}/full.mp3`, alignment: `${audioPath}/alignment.json`, mapping, key: `${audioKey}-chunk-${index}`, segment: { canonicalStart: start, canonicalEnd: start + text.length } })} className="shrink-0 rounded-md border border-blue-200 bg-blue-50 px-2 py-1 text-blue-700 hover:bg-blue-100 focus-visible:outline focus-visible:outline-2 focus-visible:outline-blue-500">🔊</button>
            <button type="button" aria-label="Close translation" onClick={() => setOpenChunk(null)} className="shrink-0 rounded-md px-2 py-1 text-violet-900 [@media(hover:hover)]:hidden">Close</button>
          </span>
        </span>
        {index < chunks.length - 1 && <span aria-hidden="true" className="mx-2 select-none text-xs text-slate-300">/</span>}
      </span>;
    })}
  </div>;
}
...
function KeyIdeaPath({ text, lang }: { text: string; lang?: 'vi' }) {
  return <div lang={lang} className="flex flex-wrap items-center gap-1.5">
    {text.split('→').map((clause, index) => <span key={index} className="inline-flex max-w-full items-center gap-1.5">
      {index > 0 && <span className="shrink-0 text-xs text-blue-400">→</span>}
      <span className={`min-w-0 break-words rounded-lg border border-blue-100 bg-white/80 px-2.5 py-1 text-sm leading-5 ${lang ? 'text-slate-700' : 'text-blue-950'}`}>{clause.trim()}</span>
    </span>)}
  </div>;
}
...
export default function BackendInterviewPage() {
  ...
  const playingThisChunk = audio.activeKey?.startsWith(`${audioKey}-chunk-`);
