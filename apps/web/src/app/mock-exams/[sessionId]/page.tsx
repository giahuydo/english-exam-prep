...
export default function MockExamSessionPage() {
  ...
  async function resume() {
    ...
    try {
      ...
      await load();
      setSession((current) =>
        current ? { ...current, ...resumed, mockStatus: 'RUNNING' } : resumed,
      );
      ...
      <div className="rounded-2xl bg-slate-950 p-5 text-white sm:p-7">
        <div className="flex flex-wrap items-start justify-between gap-4">
          ...
          <div className="text-right">
            ...
            <p className="mt-1 text-xs text-slate-400">{lifecycle}</p>
          </div>
        </div>
        <div className="mt-6">
          <ProgressHeader
      ...
      <div className="mx-auto mt-5 grid max-w-5xl gap-5 lg:grid-cols-[1fr_220px]">
        <Card className="p-6 sm:p-9">
          ...
          <div className="mt-7 grid gap-3">
            {question.options.map((option) => (
              <AnswerOption
                ...
                disabled={savingQuestionId === question.id || seconds === 0}
          ...
          <div className="mt-5 flex flex-wrap justify-between gap-3 border-t border-slate-100 pt-5">
            <Button
              className="border border-slate-200 bg-white text-slate-700 shadow-none hover:bg-slate-50"
          ...
          <div className="mt-4 grid grid-cols-5 gap-2">
            {questions.map((item, questionIndex) => (
              <button
                ...
                className={`h-9 rounded-lg border text-sm font-semibold focus:outline-none focus:ring-2 focus:ring-blue-500 ${questionIndex === index ? 'border-blue-600 bg-blue-600 text-white' : answers[item.id] ? 'border-emerald-300 bg-emerald-50 text-emerald-800' : 'border-slate-200 text-slate-600'}`}
              >
                {questionIndex + 1}
              </button>
            ))}
