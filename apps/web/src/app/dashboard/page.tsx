...
export default function DashboardPage() {
  ...
  return <StudentShell><SectionTitle eyebrow="Today" title={`Good morning, ${firstName}.`} description="Choose one focused next step. You do not need to do everything today." action={<Badge tone="blue">{data.profile.currentExamType?.name ?? 'English exam prep'}</Badge>} />
    <div className="mb-4 rounded-2xl border border-violet-200 bg-violet-50/60 p-5"><div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between"><div><p className="text-xs font-bold uppercase tracking-[.16em] text-violet-700">Exam Map / Format Memory</p><h2 className="mt-2 text-lg font-bold text-violet-950">Remember the route before you practise the question.</h2><p className="mt-1 text-sm text-violet-800">Quickly recall timing, parts, and recurring task patterns for your target exam.</p></div><Link className="inline-flex min-h-10 items-center justify-center rounded-xl bg-violet-700 px-4 text-sm font-semibold text-white hover:bg-violet-800" href="/exam-map">Open Exam Map →</Link></div></div>
