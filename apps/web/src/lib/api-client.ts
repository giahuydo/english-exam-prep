...
export const api = {
  ...
  dueLearningMemory: () => apiFetch<unknown[]>('/me/learning-memory/due'),
  listLearningMemory: () => apiFetch<unknown[]>('/me/learning-memory'),
  recordLearningMemory: (questionId: string, payload: { correct: boolean; confidence: 'KNOW' | 'UNSURE' | 'GUESS'; action?: 'ANSWERED' | 'AGAIN' | 'ADVANCE'; errorTag?: string | null }) => apiFetch<unknown>(`/me/learning-memory/questions/${questionId}`, { method: 'POST', body: JSON.stringify(payload) }),
  recordVocabularyMemory: (vocabularyKey: string, confidence: 'KNOW' | 'UNSURE' | 'GUESS', action?: 'ANSWERED' | 'AGAIN' | 'ADVANCE') => apiFetch<unknown>('/me/learning-memory/vocabulary', { method: 'POST', body: JSON.stringify({ vocabularyKey, confidence, action }) }),
